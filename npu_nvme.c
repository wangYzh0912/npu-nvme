...
typedef struct pipeline_slot
{
    ...
    // Profiling
    struct timespec t_copy_start;
    struct timespec t_copy_done;
    struct timespec t_nvme_submit;
    struct timespec t_nvme_complete;
    int chunk_seq;
    int item_idx;
    size_t item_offset;
...
static void async_pipeline_write_complete(void *arg, const struct spdk_nvme_cpl *cpl)
{
    ...
    // 更新状态
    clock_gettime(CLOCK_MONOTONIC, &slot->t_nvme_complete);
                         ...
                         size_t chunk_size_max)
{
    ...
    // 覆盖写 CSV，包含每块的详细耗时
    prof.csv = fopen("batch_profile.csv", "w");
    if (prof.csv)
    {
        fprintf(prof.csv, "chunk_seq,item_idx,item_offset,chunk_size,aligned_size,copy_ms,nvme_ms,total_ms\n");
    }
    ...
    while (batch_ctx.chunks_completed < total_chunks)
    {
        ...
        if (batch_ctx.chunks_prepared < total_chunks)
        {
            for (int slot_idx = 0; slot_idx < pipeline_depth; slot_idx++)
            {
                if (slots[slot_idx].state == SLOT_FREE)
                {
                    ...
                    clock_gettime(CLOCK_MONOTONIC, &slots[slot_idx].t_copy_start);
                    ...
                    clock_gettime(CLOCK_MONOTONIC, &slots[slot_idx].t_copy_done);
                    ...
                    slots[slot_idx].item_idx = item_idx;
                    slots[slot_idx].item_offset = item_offset;
                    slots[slot_idx].chunk_seq = chunk_seq_counter++;
        ...
        for (int i = 0; i < pipeline_depth; i++)
        {
            if (slots[i].state == SLOT_READY)
            {
                clock_gettime(CLOCK_MONOTONIC, &slots[i].t_nvme_submit);
                int rc = spdk_nvme_ns_cmd_write(
                    ctx->ns,
                    ctx->qpair,
                    slots[i].host_buffer,
        ...
        for (int i = 0; i < pipeline_depth; i++)
        {
            if (slots[i].state == SLOT_COMPLETED)
            {
                ...
                double copy_ms = timespec_diff_ms(&slots[i].t_copy_start, &slots[i].t_copy_done);
                double nvme_ms = timespec_diff_ms(&slots[i].t_nvme_submit, &slots[i].t_nvme_complete);
                double total_ms = copy_ms + nvme_ms;
                size_t aligned_size = (size_t)slots[i].num_blocks * ctx->block_size;

                prof.copy_sum += copy_ms;
                prof.nvme_sum += nvme_ms;
                if (copy_ms > prof.copy_max)
                    prof.copy_max = copy_ms;
                if (nvme_ms > prof.nvme_max)
                    prof.nvme_max = nvme_ms;
                prof.samples++;

                if (prof.csv)
                {
                    fprintf(prof.csv, "%d,%d,%zu,%zu,%zu,%.3f,%.3f,%.3f\n",
                            slots[i].chunk_seq,
                            slots[i].item_idx,
                            slots[i].item_offset,
                            slots[i].chunk_size,
    ...
    if (prof.samples > 0)
    {
        printf("[Profile] Chunks=%d\n", prof.samples);
        printf("  Copy : avg=%.3f ms, max=%.3f ms\n",
               prof.copy_sum / prof.samples, prof.copy_max);
        printf("  NVMe : avg=%.3f ms, max=%.3f ms\n",
               prof.nvme_sum / prof.samples, prof.nvme_max);
    }
                               ...
                               size_t chunk_size_max)
{
        ...
        if (chunks_prepared < total_chunks)
        {
            for (int i = 0; i < pipeline_depth; i++)
            {
                if (slots[i].state == SLOT_STATE_FREE)
                {
                    ...
                    // 设置chunk信息
                    slots[i].chunk_size = chunk_size;
                    slots[i].nvme_lba = (nvme_offsets[current_item] + current_item_offset) / ctx->block_size;
                    slots[i].num_blocks = aligned_size / ctx->block_size;
        ...
        { // 降低到30秒，更容易调试
            ...
            for (int i = 0; i < pipeline_depth; i++)
            {
                ...
                case SLOT_STATE_FREE:
                    state_str = "FREE";
                    break;
                case SLOT_STATE_COPYING_NPU:
                    state_str = "COPYING_NPU";
                    break;
                case SLOT_STATE_COPY_DONE:
                    state_str = "COPY_DONE";
                    break;
                case SLOT_STATE_NVME_SUBMITTED:
                    state_str = "NVME_SUBMITTED";
                    break;
                case SLOT_STATE_NVME_COMPLETED:
                    state_str = "NVME_COMPLETED";
                    break;
                default:
                    state_str = "INVALID";
                    break;
    ...
    for (int i = 0; i < pipeline_depth; i++)
    {
        if (slots[i].acl_event)
            aclrtDestroyEvent(slots[i].acl_event);
        if (slots[i].acl_stream)
            aclrtDestroyStream(slots[i].acl_stream);
        if (slots[i].host_buffer)
            spdk_dma_free(slots[i].host_buffer);
    }
    free(slots);
...
cleanup:
    ...
    {
        if (slots[i].acl_stream)
            aclrtSynchronizeStream(slots[i].acl_stream);
        if (slots[i].acl_event)
            aclrtDestroyEvent(slots[i].acl_event);
        if (slots[i].acl_stream)
            aclrtDestroyStream(slots[i].acl_stream);
