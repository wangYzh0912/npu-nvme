...
### B2 final acceptance (2026-09-14)

In the B2 worktree, with `PYTHONPATH=.:python` and ms_2.5:
```bash
python tools/validate_b2_acceptance.py --root /models/npu_nvme_exp/user7-stack/b2-runs --software /models/npu_nvme_exp/user7-stack/b2-runs/software-final-001 --out /tmp/b2-revalidated.json
```
Output must be new. Versioned `libnpu_nvme.so.*` paths require exact binary
hashes. Temporarily disable `set -u` while sourcing vendor CANN/conda scripts.
Failed C2 launches: `/tmp/c2-campaign.log`, `/tmp/c2-campaign-retry.log`;
successful retry: `/tmp/c2-campaign-retry2.log`.
