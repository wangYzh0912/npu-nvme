...
def main():
    ...
    try:
        ...
        # Verify checkpoint metadata is present before raw IO.
        ...
        if magic != MAGIC_NUMBER:
            raise RuntimeError("superblock magic mismatch; disk not formatted?")

        raw = RawIO(ctx)
        raw_offset = max(stack_start + 4096, META_SLOT_B_OFFSET + META_SLOT_BYTES)
        payload = b"RAW-IO-ROUNDTRIP-" + bytes(range(32))
        ...
        if (magic2, active_slot2, total2, stack2) != (magic, active_slot, total_bytes, stack_start):
        ...
        for off in (META_SLOT_A_OFFSET, META_SLOT_B_OFFSET):
            if lib.npu_nvme_sync_meta_io(ctx, off, META_SLOT_BYTES, 1,
