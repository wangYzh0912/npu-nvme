...
from npu_nvme.framework.training_state import encode_control_value, decode_control_value, restore_training_controls, capture_training_controls
...
def training_spec(components, controls, identity, *, framework, acl, npu=7, scheduler=None):
    ...
    params = capture.prepare_state_components(components)
...
class MindSporeRestoreTarget:
    ...
    def prepare_restore(self, spec):
        ...
        params = capture.prepare_state_components(self._components)
        actual = {p['name']:{k:p[k] for k in ('shape','dtype','size')} for p in params}
        if canonical(actual) != canonical(spec['parameters']):
    ...
    def apply_chunk(self, name, offset, data):
        ...
        if item['ptr']:
            ...
            rc = self.acl.aclrtMemcpy(ctypes.c_void_p(item['ptr'] + offset), len(data),
                                    ctypes.byref(buffer), len(data), 1)
            if rc != 0:
                self._unsafe.append(buffer)
                raise RuntimeError(f'restore H2D failed: {rc}')
        else:
