...
class ByteCapture:
    def __init__(self,framework,acl,manifest,registry,*,hbm_budget_bytes):
        ...
        self.current={};self.persisted={};self.host_fields=set();self.initialized=False
        for field in manifest.fields:
            if not get_dev_ptr(registry[field.canonical_name]):
                if field.byte_count>1024:raise ValueError('NPU byte capture requires large device state')
                self.host_fields.add(field.canonical_name)
                self.current[field.canonical_name]=None;self.persisted[field.canonical_name]=None
                continue
    ...
    def capture(self):
        ...
        for field in self.manifest.fields:
            ...
            if name in self.host_fields:
                self.current[name]=np.ascontiguousarray(self.registry[name].asnumpy()).tobytes()
                width=np.dtype(field.dtype).itemsize
                for block in field.blocks:
                    start=block.element_offset*width;end=start+block.element_count*width
                    flags.append(not self.initialized or self.current[name][start:end]!=self.persisted[name][start:end])
                continue
    ...
    def commit_ack(self):
        for field in self.manifest.fields:
            if field.canonical_name in self.host_fields:
                self.persisted[field.canonical_name]=self.current[field.canonical_name]
            else:
