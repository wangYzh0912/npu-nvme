...
class IncrementalState:
    def __init__(self,framework,components,region,*,host_budget_bytes,block_elements=65536,npu_capture=None):
        self.framework=framework;self.components=components;self.ready=False;self.npu_capture=npu_capture;self.failed=False
    ...
    def save(self,controls,*,step):
        if self.failed:raise RuntimeError('incremental training session failed')
        try:
            flags=self.npu_capture.capture() if self.npu_capture else None
            snapshot=self.snapshot()
            if flags is not None and self.store.ledger is not None:
                expected=[]
                for field in self.manifest.fields:
                    current=snapshot[field.canonical_name].reshape(-1)
                    previous=self.store.ledger.persisted[field.canonical_name].reshape(-1)
                    for block in field.blocks:
                        start=block.element_offset;end=start+block.element_count
                        expected.append(current[start:end].tobytes()!=previous[start:end].tobytes())
                if not np.array_equal(flags,np.asarray(expected,dtype=np.bool_)):
                    raise ValueError('NPU byte capture differs from CPU oracle')
            receipt=self.store.save(snapshot,self.encode_controls(controls),step=step,changed_blocks=flags)
            if self.npu_capture:self.npu_capture.commit_ack()
            return receipt
    ...
    def restore(self,apply_controls,verify,*,generation=None):
        ...
        verify(controls,recovered['step'])
        if self.npu_capture:
            self.npu_capture.capture();self.npu_capture.commit_ack()
        self.ready=True
        return recovered
