...
import math
import numpy as np
from .parameters import describe_tensor
...
class BlockingState:
    def __init__(self,framework,network,*,rank,executor,host_tensor_budget,partitions):
        ...
        for _,param in network.parameters_and_names():
            ...
            address=describe_tensor(param,expected_device=executor.npu_id,required_bytes=size)
            ...
            self.entries[name]=dict(param=param,pointer=pointer,address=address,dtype=dtype,shape=shape,bytes=size)
    ...
    def read_chunk(self,name,offset,length):
        ...
        if entry['pointer']:
            ...
            if self.host_source_name!=name:
                ...
                self.host_source=np.ascontiguousarray(entry['param'].asnumpy()).reshape(entry['shape'])
    ...
    def _check_address(self,entry):
        current=describe_tensor(entry['param'],expected_device=self.executor.npu_id,required_bytes=entry['bytes'])
        if current!=entry['address']:
            raise ValueError('tensor allocation changed during frozen transfer: '+entry['param'].name)
