"""Rank-local shared-slot copy wrapper; requests retain all owners until DMA safe."""
...
class RankCopyExecutor:
    def __init__(self,backend,descriptor,*,npu_id,depth,timeout_ms):
        from npu_nvme.storage.bindings import NPUNVMEContext
        ...
        self.lib.npu_nvme_init_copy_rank.argtypes=[C.POINTER(C.POINTER(NPUNVMEContext)),C.c_int,C.c_int,C.c_uint32,C.c_int,C.c_char_p,C.c_uint64,C.c_uint64]
        self.lib.npu_nvme_init_copy_rank.restype=C.c_int
        rc=self.lib.npu_nvme_init_copy_rank(C.byref(self.ctx),npu_id,depth,descriptor['chunk_bytes'],descriptor['shm_id'],descriptor['memzone'].encode(),descriptor['offset'],descriptor['length'])
        if rc:raise RuntimeError('rank copy init failed: '+str(rc))
        self.lib.spdk_memzone_lookup.argtypes=[C.c_char_p];self.lib.spdk_memzone_lookup.restype=C.c_void_p
        base=self.lib.spdk_memzone_lookup(descriptor['memzone'].encode())
        if not base:raise RuntimeError('initialized rank memzone disappeared')
        self.base=base+descriptor['offset']
    ...
    def close(self):
        with self.lock:
            ...
            self.lib.npu_nvme_cleanup(self.ctx);self.closed=True;return True
