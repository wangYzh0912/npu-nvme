...
def validate(schema):
    ...
    for row in schema['tensors']:
        ...
        if row['partition']=='per_rank_control':
            if name in tensors or row['role'] not in ('per_rank_control_scalar','runtime_rng_scalar') or not 0<row['logical_bytes_per_rank']<=1024:
...
def validate_rank(rows,schema,rank):
    ...
    required={name for name,row in expected.items() if row.get('placement')!='native_container_metadata'}
    if not required<=seen:raise ValueError('runtime missing authoritative tensors')
