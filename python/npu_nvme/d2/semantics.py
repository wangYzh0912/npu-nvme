...
def catalog(anchor,state,retention):
    ...
    for (left,_),(right,_) in zip(physical,physical[1:]):
        if left['offset']+left['length']>right['offset']:raise ValueError('overlapping physical extents')
