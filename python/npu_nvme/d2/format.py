...
def pack(kind,value,capacity):
    ...
    header=HEADER.pack(MAGIC,3,KINDS[kind],len(raw),hashlib.sha256(raw).digest())
...
def unpack(kind,raw):
    ...
    if hashlib.sha256(payload).digest()!=digest or any(raw[HEADER.size+size:]):raise ValueError('damaged page')
    def pairs(items):
        result={}
        for k,v in items:
...
class Region:
    ...
    def format(self,*,region_id):
        ...
        header=dict(format='D2',region_id=region_id,offset=self.base,length=self.end-self.base,
                    page_bytes=self.codec.page_bytes,retention=self.retention,required_flags=[],metadata_budget_bytes=self.codec.max_bytes)
    ...
    def _check_root_union(self):
        ...
        for value in sorted(refs):
            if previous and value[0]<previous[0]+previous[1]:
                if value[:2]!=previous[:2] or value[3:]!=previous[3:]:raise ValueError('conflicting anchor graphs')
            previous=value
    ...
    def _reserve(self,length):
        ...
        for index,gap in enumerate(self.pending['free']):
            if gap.length>=length:
                ...
                self.pending['allocated'].append(e);return e
        raise BufferError('retention/pins/pending generations consume the region')
    ...
    def mount(self):
        ...
        if header.get('format')!='D2' or header.get('offset')!=self.base or header.get('length')!=self.end-self.base or header.get('required_flags')!=[]:raise ValueError('unsupported region identity/flags')
        ...
        for slot in (0,1):
            try:
                ...
                semantics.catalog(anchor,state,self.retention)
                ...
                for generation in state['generations']:
                    ...
                    owned={(ref['offset'],ref['length'],ref['sha256']) for ref in generation['extents']}
                    for row in rows:
                        ref=row.get('payload')
                        if not isinstance(ref,dict) or (ref.get('offset'),ref.get('length'),ref.get('sha256')) not in owned:
    ...
    def begin(self,request_id,identity,*,retry=False):
        with self.lock:
            ...
            self.pending=dict(request_id=request_id,identity=identity,allocated=[],payload=[],free=free)
            return None
    ...
    def payload(self,data):
        with self.lock:
            ...
            try:
                ...
                self.pending['payload'].append(ref);return ref
            except BaseException:self.poisoned=True;raise

    def commit(self,rows,*,step,topology,rank_controls=None):
        with self.lock:
            if not self.pending or self.poisoned:raise RuntimeError('no writable transaction')
            try:
                self.backend.flush()
                rows=list(rows)
                owned={(ref['offset'],ref['length'],ref['sha256']) for ref in self.pending['payload']}
                for row in rows:
                    ref=row.get('payload')
                    if not isinstance(ref,dict) or (ref.get('offset'),ref.get('length'),ref.get('sha256')) not in owned:
                ...
                extent_pages=[self._put(raw) for raw in self.codec.pages(self.pending['payload'])]
                ...
                generation=dict(generation=sequence,step=step,topology=topology,pages=page_refs,extent_pages=extent_pages,control_pages=control_pages,
                    extents=self.pending['payload'],manifest_sha256=sha(canonical(rows)))
