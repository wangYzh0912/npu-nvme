...
class OwnerSession:
    def __init__(self,region,connections,*,epoch,request_id,topology,chunk_bytes,deadline,identity,authoritative_schema=None):
        ...
        self.authoritative_schema=authoritative_schema
    ...
    def schemas(self,operation):
        ...
        result=validate_schemas(schemas,self.world)
        if self.authoritative_schema is not None:
            from .tp_schema import validate_rank,digest
            if digest(self.authoritative_schema)!=self.identity.get('strategy_sha256'):raise ValueError('strategy identity differs')
            for rank,rows in schemas.items():validate_rank(rows,self.authoritative_schema,rank)
        return result
    def save(self,*,step):
