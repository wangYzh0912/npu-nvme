...
class PersistentR0:
    ...
    def save(self,state,controls,*,step,request_id=None,changed_blocks=None):
        if self.failed:raise RuntimeError('R0 owner requires fresh recovery')
        ...
        frame=self.ledger.observe(step,index,controls=[('state_controls','json',canonical(controls))],changed_blocks=changed_blocks)
        request_id=request_id or uuid.uuid4().hex
