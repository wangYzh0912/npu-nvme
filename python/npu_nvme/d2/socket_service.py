...
class RankService:
    ...
    def serve_rank(self,sock,rank):
        try:
            ...
            # Wake peers waiting for ACK or input. Socket shutdown releases no
            # native DMA buffer; each rank still executes its checked close.
    ...
    def run(self,connections,*,step,topology):
        ...
        for row in rows:
            if row['partition']=='replicated':
                key=(row['name'],row['logical_offset'])
                value=(row['logical_bytes'],row['payload']['logical_sha256'])
                if key in replicas and replicas[key]!=value:raise ValueError('replicated tensor bytes differ across ranks')
                replicas[key]=value
