...
def load_acl(acl_library="libascendcl.so"):
    """Explicit ACL dependency for capture adapters; does not open SPDK."""
    ...
    acl_lib.aclrtMemcpyAsync.argtypes = [ctypes.c_void_p, ctypes.c_size_t, ctypes.c_void_p, ctypes.c_size_t, ctypes.c_int, ctypes.c_void_p]
    acl_lib.aclrtMemcpyAsync.restype = ctypes.c_int
    acl_lib.aclrtMallocHost.argtypes = [ctypes.POINTER(ctypes.c_void_p), ctypes.c_size_t]
    ...
    acl_lib.aclrtQueryEventStatus.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_int)]
    ...
    acl_lib.aclrtEventElapsedTime.argtypes = [ctypes.POINTER(ctypes.c_float), ctypes.c_void_p, ctypes.c_void_p]
...
def load_backend(library_path=None, acl_library="libascendcl.so"):
    """Require the matching native ABI before any device initialization."""
    path = Path(library_path or Path(__file__).resolve().parents[3] / "build_out/lib/libnpu_nvme.so").resolve()
    lib = ctypes.CDLL(str(path))
    lib.npu_nvme_get_abi_version.argtypes = []
    lib.npu_nvme_get_abi_version.restype = ctypes.c_uint32
    actual = lib.npu_nvme_get_abi_version()
    if actual != ABI_VERSION:
        raise RuntimeError(f"native ABI {actual} != required {ABI_VERSION}: {path}")
    lib.npu_nvme_init.argtypes = [ctypes.POINTER(ctypes.POINTER(NPUNVMEContext)), ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.c_uint32, ctypes.c_bool, ctypes.c_char_p]
    ...
    lib.npu_nvme_get_retained_slots.argtypes = [ctypes.POINTER(NPUNVMEContext), ctypes.POINTER(NPUNVMERetainedSlot), ctypes.c_uint32, ctypes.POINTER(ctypes.c_uint32)]
    ...
    lib.npu_nvme_submit_write_batch.argtypes = [ctypes.POINTER(NPUNVMEContext), ctypes.POINTER(ctypes.c_void_p), ctypes.POINTER(ctypes.c_uint64), ctypes.POINTER(ctypes.c_size_t), ctypes.c_int, ctypes.POINTER(ctypes.POINTER(NPUNVMERequest))]
    lib.npu_nvme_submit_write_batch.restype = ctypes.c_int
    lib.npu_nvme_submit_write_batch_host.argtypes = [ctypes.POINTER(NPUNVMEContext), ctypes.POINTER(ctypes.c_void_p), ctypes.POINTER(ctypes.c_uint64), ctypes.POINTER(ctypes.c_size_t), ctypes.c_int, ctypes.POINTER(ctypes.POINTER(NPUNVMERequest))]
    lib.npu_nvme_submit_write_batch_host.restype = ctypes.c_int
    lib.npu_nvme_poll_request.argtypes = [ctypes.POINTER(NPUNVMERequest), ctypes.POINTER(ctypes.c_int)]
    lib.npu_nvme_poll_request.restype = ctypes.c_int
    lib.npu_nvme_wait_request.argtypes = [ctypes.POINTER(NPUNVMERequest), ctypes.c_uint32]
    lib.npu_nvme_wait_request.restype = ctypes.c_int
    lib.npu_nvme_release_request.argtypes = [ctypes.POINTER(NPUNVMERequest)]
    ...
    lib.npu_nvme_get_stats.argtypes = [ctypes.POINTER(NPUNVMEContext), ctypes.POINTER(NPUNVMEStats)]
    lib.npu_nvme_get_stats.restype = ctypes.c_int
    lib.npu_nvme_sync_meta_io.argtypes = [ctypes.POINTER(NPUNVMEContext), ctypes.c_uint64, ctypes.c_uint32, ctypes.c_int, ctypes.c_void_p]
    ...
    lib.npu_nvme_write_batch.argtypes = [ctypes.POINTER(NPUNVMEContext), ctypes.POINTER(ctypes.c_void_p), ctypes.POINTER(ctypes.c_uint64), ctypes.POINTER(ctypes.c_size_t), ctypes.c_int]
    lib.npu_nvme_write_batch.restype = ctypes.c_int
    lib.npu_nvme_read_batch.argtypes = [ctypes.POINTER(NPUNVMEContext), ctypes.POINTER(ctypes.c_void_p), ctypes.POINTER(ctypes.c_uint64), ctypes.POINTER(ctypes.c_size_t), ctypes.c_int]
    lib.npu_nvme_read_batch.restype = ctypes.c_int
    lib.npu_nvme_read_batch_host.argtypes = [ctypes.POINTER(NPUNVMEContext), ctypes.POINTER(ctypes.c_void_p), ctypes.POINTER(ctypes.c_uint64), ctypes.POINTER(ctypes.c_size_t), ctypes.c_int]
    lib.npu_nvme_read_batch_host.restype = ctypes.c_int
    lib.npu_nvme_write_batch_host.argtypes = [ctypes.POINTER(NPUNVMEContext), ctypes.POINTER(ctypes.c_void_p), ctypes.POINTER(ctypes.c_uint64), ctypes.POINTER(ctypes.c_size_t), ctypes.c_int]
    lib.npu_nvme_write_batch_host.restype = ctypes.c_int
    lib.npu_nvme_set_io_timeout_ms.argtypes = [ctypes.c_void_p, ctypes.c_uint32]
    ...
    return SimpleNamespace(lib=lib, acl_lib=load_acl(acl_library), library_path=str(path))
