...
from .layout import (DiskLayout, MAGIC_NUMBER, FORMAT_VERSION, SUPERBLOCK_BYTES,
    ...
    METADATA_VERSION, METADATA_FLAG_ZLIB)
...
MAX_DECODED_METADATA_BYTES = 16 << 20
...
def pack_metadata(payload: dict, generation: int) -> bytes:
    ...
    Version 1 metadata is retired. Compression is required for large
    ...
    if len(encoded) > MAX_DECODED_METADATA_BYTES:
        raise ValueError('metadata exceeds decoded byte budget')
    body_bytes = zlib.compress(encoded, level=6)
...
def unpack_metadata(raw: bytes):
    ...
    if _reserved or flags & ~METADATA_FLAG_ZLIB:
        raise ValueError('unsupported metadata flags')
    ...
    if flags & METADATA_FLAG_ZLIB:
        try:
            decoder = zlib.decompressobj()
            body = decoder.decompress(body, MAX_DECODED_METADATA_BYTES + 1)
            if len(body) > MAX_DECODED_METADATA_BYTES or not decoder.eof or decoder.unused_data:
                raise ValueError('metadata decompression exceeds budget or has trailing data')
