...
def load_config(path):
    ...
    if w['model_id'] not in ('gpt2','gpt2_xl') or w['task'] != 'causal_lm' or w['training_mode'] != 'single_rank' or w['batch'] != 1:
        raise ValueError('workload requires single-rank GPT-2/XL causal LM, batch 1')
    if w['model_id']=='gpt2_xl' and k['transport']!='async':raise CapabilityBlocked('XL regression requires explicit async transport')
    ...
    if (k['state_scope'],k['capture'],k['transport'],k['admission'],k['wait_policy'],k['retention']) != ('full_state','frozen',k['transport'],'block','source_safe',2) or k['transport'] not in ('legacy_sync','async'):
        raise CapabilityBlocked('FULL requires full_state/frozen/{legacy_sync,async}/block/source_safe/retention=2')
    ...
    max_chunk,max_depth=(16*1048576,64) if k['transport']=='async' else (1048576,16)
    if r['max_requests'] != 1 or r['chunk_bytes'] > max_chunk or r['chunk_bytes'] % 4096 or r['dma_depth'] > max_depth:
    ...
    if (s['pci'],s['namespace'],s['offset'],s['length'],s['media_version'],s['owner'],s['mode']) != ('0000:83:00.0',1,0,'device_capacity_bytes','V2','single','read_write'):
        raise ValueError('storage differs from authorized D1 namespace/geometry')
    if type(s['slot_size_gb']) is not int or s['slot_size_gb']<=0:raise ValueError('slot capacity must be explicit positive GiB')
    if k['transport']=='legacy_sync' and s['slot_size_gb']!=10:raise ValueError('archived C1 slot geometry is frozen')
