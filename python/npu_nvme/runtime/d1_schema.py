...
def spec_validate(spec):
    ...
    for name, info in params.items():
        if not isinstance(name, str) or not name.startswith(('model/', 'optimizer/')) or len(name.encode()) > 1024:
...
def validate_record(record, layout, expected_spec=None):
    ...
    chunk_size = integer(record.get('chunk_size'), 'chunk size', 4096, CHUNK_BYTES)
    if chunk_size % 4096:
        raise ValueError('chunk size is unaligned')
    if not isinstance(record.get('request_id'), str) or not record['request_id'] or not isinstance(record.get('writer_epoch'), str):
    ...
    for name, info in params.items():
        ...
        elif any(info[k] != spec['parameters'][name][k] for k in ('shape', 'dtype', 'size')):
