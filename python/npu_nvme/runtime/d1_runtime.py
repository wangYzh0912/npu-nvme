...
class FullRuntime:
    def __init__(self, coordinator, transport, *, prepare, freeze, release, chunk_size=CHUNK_BYTES):
        ...
        self.restore_session = StrictRestoreSession(coordinator=coordinator, reader=transport.read,
            ...
            applier=getattr(transport,'apply_chunk',None))
        ...
        try:
            ...
            for item in items:
                ...
                for chunk in info['chunks']:
                    if hasattr(self.transport,'write_frozen'):
                        digest=self.transport.write_frozen(info['offset']+chunk['offset'],item,chunk['offset'],chunk['size'])
                        if digest!=chunk['sha256']: raise ValueError('frozen source changed during transport')
                    else:
