...
from experiments.baselines.repro.protocol import AdapterError, DependencyBlocked, EventLog
from experiments.baselines.repro.state_bridge import (apply_snapshot, capture_snapshot, create_batches,
                           load_raw_snapshot, save_raw_snapshot, write_json)
from python.training_state import capture_training_controls, restore_training_controls, encode_control_value, decode_control_value
from npu_nvme.cli.contracts import loss_matches, canonical
...
def _model(config, initialized=False):
    ...
    cfg = model.config.to_dict() if hasattr(model.config, 'to_dict') else vars(model.config)
    resolved = json.loads(json.dumps(cfg, sort_keys=True, default=str))
    config['resolved_model_config'] = resolved
    revision = 'sha256:' + canonical(resolved)
    declared = config.get('model_revision', '')
    if declared.startswith('sha256:') and revision != declared:
        raise ValueError('resolved GPT-2 model configuration differs from locked revision')
    config['observed_model_revision'] = revision
...
def _step(ms, cell, batch_path, step):
    from experiments.baselines.repro.state_bridge import load_batch
...
def prepare(config, root):
    ...
    return {"model_revision": config.get("observed_model_revision"), "fixture": str(fixture_dir), "batches": batches,
...
def run(config, adapter_name, run_dir, fixture_dir=None, batches_path=None):
    ...
    ms, model, optimizer, cell = _strict_model(config)
    ...
    try:
        ...
        adapter.prepare(fixture.schema, config)
        records = []
        checkpoints = []
        ...
        for formal_step in range(1, int(config["formal_steps"]) + 1):
            ...
            if adapter_name != "none" and formal_step % int(config["checkpoint_every"]) == 0:
                if checkpoints:
                    adapter.wait_persisted(checkpoints[-1]['handle'],config['timeout_seconds'])
                    adapter.prune(2)
        ...
        if adapter_name != "none":
            adapter.drain(config.get("drain_timeout_seconds",config["timeout_seconds"]))
            adapter.prune(2)
            adapter.close()
            materialized = []
        ...
        training_end = time.monotonic_ns()
        ...
        result = {
            ...
            "total_wall_seconds": (training_end - start) / 1e9,
            "oracle_seconds": (time.monotonic_ns() - training_end) / 1e9,
            "transport": config.get("transport","async") if adapter_name=="ours" else "framework", "capture": "frozen",
            "deterministic": "ON", "source_pid": os.getpid(),
            "state_bytes": fixture.total_bytes,
            "initial_state_sha256": fixture.digest(),
            "model_revision": config.get("observed_model_revision"),
            "resolved_model_config": config.get("resolved_model_config"),
        }
...
def restore(config, adapter_name, run_dir, generation="latest-committed",
    ...
    restore_result = {}
    try:
        mark("restore_begin", selected_generation=int(selected["generation"]), selected_step=int(selected["step"]))
        ...
        ms.hal.synchronize()
        mark("integrity_and_apply_done", mandatory_checksums=True)
        expected_controls = decode_control_value(snapshot.controls_payload, snapshot.controls_metadata) if snapshot is not None else restored
        actual_controls = capture_training_controls(ms, optimizer, applied['data_cursor'],
            expected_controls['loss_scale'], int(config['seed']))
        if any(encode_control_value(actual_controls[k])[0].tobytes() != encode_control_value(expected_controls[k])[0].tobytes() for k in actual_controls):
            raise ValueError('control readback mismatch')
        if applied['data_cursor']['next_step'] != int(selected['step'])+1:
            raise ValueError('restored data cursor differs from selected step')
        mark("controls_verified")
        mark("state_ready")
        ...
        trajectory = {int(row["step"]):row["loss"] for row in result.get("steps", []) + result.get("source_oracle", [])}
        oracle = [trajectory[x] for x in range(final_step+1,final_step+steps+1) if x in trajectory]
        deviations = [abs(float(a) - float(b)) for a, b in zip(losses, oracle)]
        loss_ok = loss_matches(losses, oracle, steps, float(config['loss_rtol']), float(config['loss_atol']))
        restore_result = {
            ...
            "mandatory_integrity": True, "controls_verified": True,
            "restore_pid": os.getpid(), "source_pid": result.get("source_pid"),
            "byte_exact": byte_exact,
        ...
        restore_result = {"status": "restore_failed", "adapter": adapter_name,
                          "error": repr(error), "error_code": 4 if isinstance(error, TimeoutError) else 5 if isinstance(error, ValueError) else 1, "timing": {"events": events},
                          "mode": mode, "repeat_index": repeat_index}
    finally:
        if adapter is not None:
            try:
                ...
                restore_result["status"] = "restore_failed"
                restore_result["error_code"] = 4
                restore_result["requires_reconcile"] = True
