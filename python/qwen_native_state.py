...
def fixed_step_checkpoint_monitor(base):
    """Keep upstream synchronous saves; omit its extra forced end-of-run save."""
    class FixedStepCheckpointMonitor(base):
        def __init__(self, **kwargs):
            if kwargs.get('async_save', False):
                raise ValueError('fixed-step baseline requires synchronous Native save')
            super().__init__(**kwargs)

        def on_train_end(self, run_context):
            # Upstream on_train_end forces an additional checkpoint at step 11.
            # The comparison contract has exactly one checkpoint, at step 8.
            pass
    return FixedStepCheckpointMonitor
