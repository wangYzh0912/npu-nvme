...
def checkpoint(framework, network, *, operation, rank, out, source=None,
               ...
               host_budget=128 << 30, generation=8, step=8):
    ...
    if operation not in ('save','restore'):
        raise ValueError('unsupported ByteCheckpoint operation')
    if type(generation) is not int or generation<1 or type(step) is not int or step<1:
        raise ValueError('positive generation and optimizer step required')
    strategy = json.loads((ROOT/'config/qwen_runtime_schema.json').read_text())
    ...
    if operation == 'restore':
        ...
        if saved.get('generation',8)!=generation or saved.get('step',8)!=step:
            raise ValueError('ByteCheckpoint generation/step differs')
    ...
    try:
        if operation == 'save':
            ...
            for field in sorted(fields, key=lambda value: value['name']):
                ...
                array = np.ascontiguousarray(param.asnumpy()).reshape(field['shape'])
        ...
        with (directory/f'bytecheckpoint-{operation}.stderr.log').open('w') as log:
            ...
            request = dict(operation=operation, generation=generation, checkpoint_dir=str(storage),
        ...
        if operation == 'save':
            ...
            framework.hal.synchronize(); verify(restored, step); barrier()
        return dict(receipt=dict(generation=generation), capture='blocking_host_snapshot',
