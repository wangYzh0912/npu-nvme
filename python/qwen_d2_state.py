...
def validate_restart(run,rank,step,horizon):
    ...
    for row in rows:
        ...
        actual=set(row['artifacts'])
        if contract.get('backend','d2')=='bytecheckpoint_host':
            if not required<=actual or f'rank_{row["rank"]}/bytecheckpoint/bridge.json' not in actual:
                raise ValueError('ByteCheckpoint artifact coverage differs')
            if any(not name.startswith(f'rank_{row["rank"]}/bytecheckpoint/') for name in actual-required):
                raise ValueError('ByteCheckpoint artifact escapes rank')
        elif actual!=required:raise ValueError('D2 source artifact set differs')
        for name,digest in row['artifacts'].items():
            path=(run/name).resolve()
            if run.resolve() not in path.parents:raise ValueError('D2 source artifact path escapes run')
            if row['rank']==rank or '/bytecheckpoint/' not in name:
                hasher=hashlib.sha256()
                with path.open('rb') as stream:
                    for block in iter(lambda:stream.read(8<<20),b''):hasher.update(block)
                if hasher.hexdigest()!=digest:raise ValueError('D2 source artifact changed')
    return contract
