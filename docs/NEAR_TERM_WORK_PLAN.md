...
第一轮 Go 条件为至少一组在 FULL 摊销后物理写量低于 FULL-only 的 20%，单步 NRMSE
中位数不高于 5e-3，无超龄块，固定批次恢复 loss 偏差不高于 1%。否则立即 Pivot 到
...
GPT-2 三 seed 正式轨迹已完成（seed 41/42/43，100 steps，seq_len 1025）。跨 seed 的
Top-10%/Top-20% 变化能量覆盖中位数如下：

| block size | Top-10% | Top-20% | 选块 Jaccard 中位数 |
|---:|---:|---:|---:|
| 65536 | 88.56% | 92.90% | 0.0557 |
| 262144 | 87.56% | 92.23% | 0.0699 |
| 524288 | 87.75% | 92.63% | 0.0741 |

以 262144 elements 为例，Adam m 约占变化能量 91.70%，其 Top-10% 覆盖约 87.67%；模型
权重约占 8.30%，Top-10% 仅约 24.98%。因此“变化能量集中”在 GPT-2 100-step 轨迹上
得到支持，但固定 Top-K 仍存在未选块年龄达到 100 的饿死问题，必须依赖 max-age。
完整轨迹证据已提交于 `results/incremental-next-20260828/trajectory-gpt2-seed*/`。

基于旧的一步/末尾 FULL 测试得到的低写量候选不能作为最终 Go 证据；新的 R2 回放已改为
在 step 99 的 Delta 状态恢复并连续比较 10 个训练步，结果完成前不进入异步/NPU 融合。

三 seed 的精确 R2 回放已完成，机器可读聚合证据为
`results/incremental-next-20260828/decision-gpt2-v2/result.json`，策略结果为
`policy-gpt2-seed41/42/43-v2/result.json`。聚合状态为 **PIVOT**，三个候选均未同时满足
写量、单步误差、恢复误差和年龄门禁：

| 候选 | 写入比例中位数/最大值 | 单步 NRMSE 中位数 | 10 步恢复 loss 最大相对误差 | 最大分类误差（Adam m） | 结果 |
|---|---:|---:|---:|---:|---|
| 262K / Top-5% / INT8 / FULL-100 / age-8 | 4.989% / 4.999% | 6.608e-3 | 0.636% | 1.012 | 单步误差超 5e-3 |
| 262K / error-budget-90% / INT8 / FULL-50 / age-8 | 11.141% / 11.149% | 6.401e-3 | 0.175% | 0.923 | 单步误差超 5e-3 |
| 65K / Top-20% / FP16 / FULL-20 / age-4 | 20.752% / 20.754% | 3.10e-4 | 6.730% | 0.350 | 写量和恢复误差超门限 |

这次结果支持“变化能量存在集中性”，但不支持当前 R2 候选已经具备可用的完整训练
状态恢复质量：低写量方案主要牺牲 Adam m 状态，整体 NRMSE 和 loss 不能掩盖该分类
漂移；较低误差方案又无法满足写量或长期恢复门限。因此按预定规则暂停 NPU 图内 R2
融合、连续 HBM buffer、`aclrtMemcpyAsync` 和 13B 扩展，Pivot 方向为状态分类策略：
优先验证 Adam m/v 保守刷新或低频 FULL、权重单独增量及低比特全量编码，再决定是否恢复
NPU/异步优化投入。

GPT-2 XL 500-step 真实轨迹此前在 NPU7 长跑，采用 1～30、236～265、471～500 采样窗；
中段稀疏采样期间报告的 age 是“采样观察间隔”而非每一步真实 age，不能作为正式
max-age 结论。最终结果见下文。

GPT-2 XL 轨迹已完成：seed 42、500 steps、seq_len 1025，三个采样窗口共 90 个样本，
`failed_samples=0`，数值门禁通过。机器可读证据位于
`results/incremental-next-20260828/trajectory-gpt2xl-seed42-light/`。以三种块大小统计
观察到的 Top-K 变化能量和相邻样本选块 Jaccard 如下：

| block size | Top-10% 覆盖中位数 | Top-20% 覆盖中位数 | Jaccard 中位数 | Top-10% frame 末样本 |
|---:|---:|---:|---:|---:|
| 65536 | 92.19% | 98.21% | 0.0546 | 1,029,445,349 B |
| 262144 | 89.65% | 98.11% | 0.0413 | 1,031,095,034 B |
| 524288 | 88.55% | 98.02% | 0.0400 | 1,026,935,346 B |

完整状态大小为 9,822,624,008 B，因此末样本 Top-10% frame 约为 FULL 状态的 10.5%；
这是基于轨迹的逻辑 frame 估计，不是实际 R2 磁盘写入收益。按 262K block 从每个样本
的分类 L2 推导，模型权重和 Adam m 分别约贡献 47.16%/52.84% 的相邻变化能量，Adam v
能量约为 `2.2e-8`；分类相对 L2 中位数约为 model `2.25e-4`、Adam m `2.32e-2`、
Adam v `0`。这说明 XL 也存在可利用的能量集中性，但 Adam m 仍是主要误差风险，不能
直接套用 GPT-2 的低写量策略。

该实验每步训练但只在三个窗口记录样本，窗口间没有持久化参考更新；因此输出中的最大
