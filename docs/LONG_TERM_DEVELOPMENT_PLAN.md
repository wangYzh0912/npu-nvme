...
| C1 / B2 / C2 | planned | D1 入口已闭合；统一训练入口、公平恢复、双方向异步传输及 C ABI 退役按各自门禁推进 |
...
## 2026-09-14：旧实现清理支线（已批准的替代策略）

执行细目与验收见 [旧实现清理](LEGACY_IMPLEMENTATION_CLEANUP.md)。本支线替代 C1 的旧入口转发、旧 Python API 报错壳和独立探索算法保留策略：调用方迁移后直接删除，归档标签为 `archive-exploratory-implementations-20260914`。当前唯一训练入口为 `train.py`，原生 ABI 为 2。live、Delta/R0、多 rank 目标保留，旧算法及其专用测试离开活动分支。当前严格 FULL 所需 C batch 接口仍按 B2/H07→C2 门禁退役；V2 介质布局不变，D2 格式升级独立执行。
