...
### Software completion after retention

- Frozen `1920cb5`, `final-software-c2-20260915-003`: all 295 declared C2 checks passed, including the C safety case; no source drift. This is software evidence only.
- Focused D2/E1/campaign suite: 76 passed. Native fixed-step policy and training entry checks: 17 passed. Counts overlap; do not sum them as independent acceptance cases.
- Native's upstream `on_train_end` forces an extra checkpoint. The fixed-step adapter preserves upstream synchronous periodic saving at step 8 and omits the forced step-11 save. Fresh restore disables checkpoint writing; its checker verifies the selected source checkpoint and recovered state/continuation. Hardware evidence is pending.
- Next execution order after proven device recovery: new-library TP2/TP4 sessions with exact caller affinity checks; corrected four-method pilot; formal repeated Qwen campaign; E1 GPT-2 then XL and F1; scratch faults then strict V2 migration; final gates and switchback. Recheck filesystem space for all retained sources, converted weights, temporary Native/ByteCheckpoint writes and oracles before the repeated campaign.
