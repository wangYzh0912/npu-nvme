...
from user_environment import isolated_environment, read_profile, verify_library_paths, verify_runtime_abi, with_python_identity
...
class UserEnvironmentTests(unittest.TestCase):
    ...
    def test_stage_library_override_is_fingerprinted(self):
        stage = self.toolkit / 'lib64/stage.so.2'
        stage.write_bytes(b'ABI2')
        observed = read_profile(self.manifest_path, 'old', self.root/'repo', library_override=stage)
        self.assertEqual(observed['library'], str(stage))
        self.assertNotEqual(observed['environment_id'], self.profile()['environment_id'])

    def test_required_abi_rejects_old_runtime(self):
        profile = dict(self.profile(), required_abi_major=2)
        with patch('user_environment.subprocess.run', return_value=subprocess.CompletedProcess([],0,'{"abi_major":1}','')):
            with self.assertRaisesRegex(ValueError, 'ABI differs'):
                verify_runtime_abi(profile, {})

    def test_required_abi_rejects_missing_symbol(self):
        profile = dict(self.profile(), required_abi_major=2)
        with patch('user_environment.subprocess.run', return_value=subprocess.CompletedProcess([],1,'','undefined symbol')):
            with self.assertRaisesRegex(ValueError, 'load probe failed'):
                verify_runtime_abi(profile, {})
