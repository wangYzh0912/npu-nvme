...
from npu_nvme.storage.bindings import (NPUNVMETransferSpec, NPUNVMETransferItem,
    NPUNVMETransferReceipt, NPUNVMETransferDigest, NPUNVMECapabilities, NPUNVMECopySpec)


def test_c_python_transfer_layout(tmp_path):
    root=Path(__file__).resolve().parents[2]
    types=[NPUNVMECopySpec,NPUNVMECapabilities,NPUNVMETransferSpec,NPUNVMETransferItem,NPUNVMETransferReceipt,NPUNVMETransferDigest]
    statements=[];expected=[]
    for typ in types:
