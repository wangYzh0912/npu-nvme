...
int main(int argc, char **argv) {
    ...
    if (!strcmp(argv[1], "closed_admission")) {
    ...
    } else if (!strcmp(argv[1], "ring_bounds")) {
        ring_t ring; int value;
        assert(ring_init(&ring, 5) == 0);
        assert(ring_is_empty(&ring));
        for (int cycle = 0; cycle < 3; cycle++) {
            for (int i = 0; i < 4; i++) assert(ring_push(&ring, cycle * 4 + i) == 0);
            assert(ring_is_full(&ring) && ring_push(&ring, 99) == -1);
            for (int i = 0; i < 4; i++) { assert(ring_pop(&ring, &value) == 0); assert(value == cycle * 4 + i); }
            assert(ring_is_empty(&ring) && ring_pop(&ring, &value) == -1);
        }
        ring_free(&ring);
