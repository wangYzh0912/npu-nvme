...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--phase',choices=['inspect','format','baseline','source','restore'],required=True)
    ...
    if a.phase in ('inspect','format'):
        ...
        try:
            ...
            for start,end in ((layout.full_base,layout.full_end),(layout.delta_base,layout.delta_end)):
                for protected_start,protected_end in ((256<<30,1280<<30),(1280<<30,1408<<30),(1536<<30,1664<<30)):
                    if start<protected_end and protected_start<end:raise ValueError('E1 extent overlaps registered D2 region')
            if layout.full_end>128<<30:raise ValueError('E1 FULL exceeds legacy area')
            header=transport.read(0,1<<20)
            digest=hashlib.sha256(header).hexdigest()
            if a.phase=='inspect':
                (a.out/'v2-header-before.bin').write_bytes(header)
                write(a.out/'inspect.json',dict(status='pass',header_sha256=digest,pci=a.pci))
                return
            inspected=json.loads((a.out/'inspect.json').read_text())
            if inspected.get('status')!='pass' or inspected.get('pci')!=a.pci or inspected.get('header_sha256')!=digest:
                raise ValueError('legacy header changed after inspection; refusing format')
