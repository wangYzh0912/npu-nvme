...
def device_write(ctx, ptrs, offsets, sizes, api):
    if api == 'batch':
        return lib.npu_nvme_write_batch(ctx, ptrs, offsets, sizes, 1)
    request = ctypes.POINTER(NPUNVMERequest)()
    rc = lib.npu_nvme_submit_write_batch(ctx, ptrs, offsets, sizes, 1, ctypes.byref(request))
    if rc != 0:
        return rc
    # An observation deadline cannot release source ownership. Fault-timeout
    # behavior is covered separately by the lifecycle harness.
    try:
        while True:
            rc = lib.npu_nvme_wait_request(request, 120000)
            done = ctypes.c_int()
            status = lib.npu_nvme_poll_request(request, ctypes.byref(done))
            if done.value:
                return status
    finally:
...
def hbm_matrix(ctx, start_offset, args):
        ...
        try:
            ...
            for inner in range(0, size, args.chunk):
                ...
                ptrs, offsets, sizes = arrays(source_dev.value + inner,
                                              offset + inner, part)
                require(device_write(ctx, ptrs, offsets, sizes, args.api) == 0,
...
def main():
    ...
    parser.add_argument("--api", choices=("batch", "request"), default="request")
    ...
    require(args.pci == "0000:83:00.0", "only authorized scratch namespace may be written")
    ...
    try:
        ...
        next_offset = hbm_matrix(first, next_offset, args)
    ...
    Path(args.output, 'result.json').write_text(json.dumps({'status':'pass',
        'scope':'Host and HBM raw transfer, partial chunks, bounds, reopen',
        'device_write_api':args.api, 'elapsed_seconds':time.monotonic()-started,
        'native_library':backend.library_path,
        'library_sha256':hashlib.sha256(Path(backend.library_path).read_bytes()).hexdigest()}, indent=2)+'\n')
