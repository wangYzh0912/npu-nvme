...
from npu_nvme.storage.bindings import (load_backend, NPUNVMEContext, NPUNVMERequest,
    ...
    NPUNVMETransferDigest, NPUNVMEStats, NPUNVMECapabilities)
...
def worker(a):
    ...
    try:
        ...
        if a.read:
            ...
            check(lib.npu_nvme_read_batch_host(ctx,ptrs,offsets,sizes,len(parts))==0,'readback')
        ...
        if a.kind=='hbm':check(counters['async_dma_submit_count']==len(parts)*(2 if a.read else 1),'async D2H path absent')
