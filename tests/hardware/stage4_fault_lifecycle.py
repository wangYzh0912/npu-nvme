...
class Worker:
    def __init__(self, args, record):
        from npu_nvme.storage.bindings import (load_backend, NPUNVMEContext,
            NPUNVMERequest, NPUNVMEStats, NPUNVMERetainedSlot, NPUNVMECapabilities)
        ...
        self.capability_type = NPUNVMECapabilities
        self.max_pending = 16 # archived libraries without capability query
    ...
    def open(self):
        ...
        if hasattr(self.lib,"npu_nvme_get_capabilities"):
            caps=self.capability_type()
            require(self.lib.npu_nvme_get_capabilities(self.ctx,ctypes.byref(caps),ctypes.sizeof(caps))==0,"capability query failed")
            self.max_pending=caps.max_pending_requests
            self.record["capabilities"]={name:int(getattr(caps,name)) for name,_ in caps._fields_}
    ...
    def host_write(self):
        value = self.buffer()
        return transfer_wait(self.lib,0,1,self.ctx,
    ...
    def verify(self):
        ...
        rc = transfer_wait(self.lib,1,1,self.ctx, *self.arrays(ctypes.addressof(value)), 1)
    ...
    def backpressure(self):
        ...
        busy = 0
        for _ in range(self.max_pending+2):
    ...
    def close(self):
        ...
        require(snap["stats"]["dma_inflight_peak"] <= self.args.depth and
                ...
                snap["stats"]["request_ring_peak"] <= self.max_pending, "resource peak exceeds configured bounds")
        for request in self.requests:
            done = ctypes.c_int()
