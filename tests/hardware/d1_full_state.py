...
from c1_training_state_restart import (build_training, train_range, state_digest, control_state,
    write_state_oracle, compare_state_oracle)
...
def open_store(args):
    ...
    return DirectCheckpoint(strict=True,nvme_addr=args.pci,npu_device_id=args.npu,
        requested_chunk_size=1<<20,keep_last_n=2,slot_size_gb=10,pipeline_depth=4,
        spdk_shm_id=args.shm_id,profiling_dir=str(args.out/'profiling'))
...
def main():
    ...
    sources={str(f.relative_to(ROOT)):sha256_file(f) for directory in ('python','src','include','tests')
