...
def main():
    ...
    a.npu=7;a.model='gpt2';a.seq_len=129;a.dropout_rate=0.;a.loss_scale=1.;a.deterministic='ON';a.save_step=5;a.continue_steps=3
    ...
    try:
        ...
        adapter.npu_capture=ByteCapture(ms,load_backend().acl_lib,adapter.manifest,adapter.registry,hbm_budget_bytes=16<<30)
        if a.phase=='source':
            receipts=[]
            ...
            controls=control_state(ms,optimizer,5,a);hashes={k:encode_control_value(v)[1]['sha256'] for k,v in controls.items()}
            losses,_=train_range(ms,cell,6,8,a.seq_len);write_state_oracle(a.out/'oracle',model,optimizer)
            write(a.out/'source.json',dict(status='pass',receipts=receipts,state=checkpoint,controls_sha256=hashes,losses=losses,state_bytes=adapter.state_bytes,
                capture='NPU byte mirrors with CPU exact oracle',max_delta_chain=2,manifest_sha256=adapter.manifest.digest))
        else:
            ...
            def verify(value,step):
                assert step==5 and state_digest(model,optimizer)==saved['state']
            ...
            losses,_=train_range(ms,cell,6,8,a.seq_len)
            ...
            post_receipt=adapter.save(control_state(ms,optimizer,8,a),step=8)
            replay=adapter.store.recover(post_receipt['generation'])
            observed=adapter.snapshot()
            assert all(observed[k].tobytes()==v.tobytes() for k,v in replay['state'].items())
            write(a.out/'restore.json',dict(post_restore_receipt=post_receipt,status='pass',ready=True,loaded_byte_exact=True,controls_byte_exact=True,comparison=comparison,losses=losses,generation=result['generation']))
    finally:
