...
def test_external_bitmap_must_match_exact_byte_changes_before_commit():
    disk,manifest,store=setup();state={'model/weight':np.zeros(4096,np.float32)}
    store.save(state,{},step=1)
    state['model/weight'][0]=-0.0
    writes=len(disk.writes)
    with pytest.raises(ValueError,match='bitmap differs'):
        store.save(state,{},step=2,changed_blocks=np.zeros(4,np.bool_))
    assert len(disk.writes)==writes and store.ledger.persisted_generation==0
    store.save(state,{},step=2,changed_blocks=np.array([True,False,False,False]))
    assert remount(disk,manifest).recover()['state']['model/weight'].tobytes()==state['model/weight'].tobytes()
