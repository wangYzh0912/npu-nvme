...
def state(monkeypatch,pointer=0,budget=20):
    monkeypatch.setattr('npu_nvme.framework.d2_state.get_dev_ptr',lambda _:pointer)
    ...
    return BlockingState(ms,network,rank=0,executor=None,host_tensor_budget=budget,partitions={'x':'replicated'}),param
