...
class Symbol(SimpleNamespace):
    def __call__(self,*args): return 2
...
class Symbols:
    ...
    def __getattr__(self,name):
        ...
        return self.symbols.setdefault(name,Symbol())


@pytest.mark.parametrize('symbol',['npu_nvme_init','npu_nvme_flush','npu_nvme_close','npu_nvme_submit_transfer','npu_nvme_submit_copy','npu_nvme_get_capabilities','npu_nvme_wait_quiescent'])
...
@pytest.mark.parametrize('symbol',['npu_nvme_submit_write_batch_host','npu_nvme_delta_get_area_offset','npu_nvme_get_io_timeout_ms','npu_nvme_register_tasks'])
