...
def test_d1_reservation_is_single_owner_and_unknown_is_explicit():
    ...
    c = D1CommitCoordinator(metadata_io=IO(), state=MetadataState())
    ...
    out = c.publish(r, TransferReceipt(r.request_id,r.generation,4,1,True,'a'*64))
    assert c.resolve(r.request_id) == out
