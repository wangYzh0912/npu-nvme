...
def manager(monkeypatch, slots=1):
    ...
    m.admission = 'block'; m._queue_poisoned = False; m._closing = m._closed = False
    m.checkpoint_slots = m.request_slots = slots
    m._slot_sem = threading.BoundedSemaphore(slots); m._request_sem = threading.BoundedSemaphore(slots)
    m._admission_lock = threading.Lock(); m._admission_changed = threading.Condition(m._admission_lock)
    m._leases = set(); m._handles_lock = threading.Lock(); m._active_handles = set(); m._handle_threads = {}
    m._io_order = threading.Condition(); m._io_mutex = threading.Lock(); m._sequence_lock = threading.Lock()
    m._next_io_sequence = 1; m._request_counter = m._snapshot_generation = m._accepted_generation = 0
...
def test_second_budget_failure_leaves_request_budget_available(monkeypatch):
    ...
    assert m._request_sem.acquire(blocking=False)
...
def test_lease_cannot_release_twice(monkeypatch):
    ...
    assert m._slot_sem.acquire(blocking=False)
    assert not m._slot_sem.acquire(blocking=False)
...
def test_preworker_failures_release_only_owned_resources(monkeypatch,fault):
    ...
    assert not m._leases and not m._active_handles and not m._handle_threads
    assert m._request_sem.acquire(blocking=False) and m._slot_sem.acquire(blocking=False)
...
def test_barrier_includes_capture_not_yet_registered_as_worker(monkeypatch):
    ...
    assert not t.is_alive() and len(errors)==1 and not m._leases
...
def test_reset_returns_acknowledged_failure(monkeypatch):
    ...
    failure=RuntimeError('prior write failed');m._io_error=failure;m._queue_poisoned=True
...
def test_quarantine_has_bounded_owned_byte_ledger(monkeypatch):
    ...
    assert m._leases=={lease} and not m.released
