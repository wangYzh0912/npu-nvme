...
static int submit_write_owned(NPUNVMEContext *ctx, void **ptrs,
    ...
    *out_request = NULL;
    if (ctx && ctx->role==NPU_NVME_ROLE_HOST_OWNER && !is_host) return -ENOTSUP;
...
int npu_nvme_submit_transfer(NPUNVMEContext *ctx,
    ...
    *out = NULL;
    ...
    if (!ctx) return -EINVAL;
    if (ctx->role==NPU_NVME_ROLE_COPY_RANK ||
        (ctx->role==NPU_NVME_ROLE_HOST_OWNER && spec->memory_kind!=NPU_NVME_MEMORY_HOST)) return -ENOTSUP;
...
int npu_nvme_submit_copy(NPUNVMEContext *ctx, const NPUNVMECopySpec *spec,
    ...
    if (!out) return -EINVAL;
    *out=NULL;
    if (ctx && ctx->role==NPU_NVME_ROLE_HOST_OWNER) return -ENOTSUP;
    if (!ctx || !spec || spec->struct_size!=sizeof(*spec) || spec->version!=1 ||
        (spec->operation!=NPU_NVME_COPY_D2H && spec->operation!=NPU_NVME_COPY_H2D) ||
        spec->reserved || (spec->checksum_flags & ~3U) || !spec->source || !spec->destination ||
        !spec->length || spec->length>ctx->dma.chunk_size || spec->length>SIZE_MAX ||
        (uintptr_t)spec->source>UINTPTR_MAX-spec->length ||
        (uintptr_t)spec->destination>UINTPTR_MAX-spec->length) return -EINVAL;
    if (atomic_load(&ctx->quarantined)) return -EIO;
    if (atomic_load(&ctx->admission_closed) || atomic_load(&ctx->app_should_stop)) return -ESHUTDOWN;
    write_request_t *req=calloc(1,sizeof(*req));
    if (!req) return -ENOMEM;
    bool d2h=spec->operation==NPU_NVME_COPY_D2H;
    void *device=d2h ? spec->source : spec->destination;
    uint64_t offset=0; size_t size=spec->length;
    req->tasks=create_io_tasks(1,&device,&offset,&size);
    if (!req->tasks) {free(req);return -ENOMEM;}
    req->tasks[0].host_ptr=d2h ? spec->destination : spec->source;
    req->tasks[0].expected_crc32=spec->expected_crc32;
    memcpy(req->tasks[0].expected_sha256,spec->expected_sha256,32);
    req->ctx=ctx;req->num_tasks=1;req->is_copy=true;req->async_dma=true;
    req->operation=spec->operation;
    req->compute_crc=spec->checksum_flags & NPU_NVME_CHECK_CRC32;
    req->compute_sha256=spec->checksum_flags & NPU_NVME_CHECK_SHA256;
    req->timeout_ms=finite_timeout(ctx->io_timeout_ms);
    identify_tasks(ctx,req->tasks,1);
    atomic_init(&req->done,0);atomic_init(&req->detached,0);atomic_init(&req->refs,2);
    atomic_fetch_add(&ctx->refs,1);
    int rc=enqueue_request(ctx,d2h ? 0 : 1,req);
    if (rc) {write_request_put(req);write_request_put(req);return rc;}
    *out=req;return 0;
}
