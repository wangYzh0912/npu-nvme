...
int ensure_acl_context(NPUNVMEContext *ctx) {
    if (ctx->role == NPU_NVME_ROLE_HOST_OWNER) return 0;
...
int npu_nvme_get_capabilities(NPUNVMEContext *ctx, NPUNVMECapabilities *out, uint32_t size) {
    ...
    uint64_t capacity=0;
    if (ctx->role != NPU_NVME_ROLE_COPY_RANK && namespace_capacity(ctx,&capacity)) return -EINVAL;
    *out=(NPUNVMECapabilities){.struct_size=sizeof(*out),.version=1,
        .operation_mask=ctx->role==NPU_NVME_ROLE_COPY_RANK ? (1U<<5)|(1U<<6) :
            ctx->role==NPU_NVME_ROLE_HOST_OWNER ? (1U<<5)-1 : (1U<<7)-1,
...
static int initialize_role(NPUNVMEContext **out_ctx, const char *pci_addr, int npu_id,
                  int pipe_depth, uint32_t chunk_size, bool enable_profiling,
                  const char *prof_dir, uint32_t role) {
    ...
    *out_ctx = NULL;
    ...
    char rank_lock[64];
    snprintf(rank_lock,sizeof(rank_lock),"ffff:%02x:00.0",npu_id);
    int owner_rc = acquire_native_owner(ctx, role==NPU_NVME_ROLE_COPY_RANK ? rank_lock : pci_addr);
     ...
     * DPDK EAL to be fully initialised. */
    {
        ...
        static uint32_t initialized_role;
        if (spdk_inited && initialized_role != role) {
            npu_nvme_cleanup(ctx); return -EINVAL;
        }
        if (!spdk_inited) {
            struct spdk_env_opts env_opts = {.opts_size=sizeof(env_opts)};
            spdk_env_opts_init(&env_opts);
            env_opts.name = "npu_nvme_app";
            env_opts.no_pci = role==NPU_NVME_ROLE_COPY_RANK;
            ...
            /* EAL pins its calling thread to the main lcore. This is an
             * embedded library: do not pin the framework and future rank
             * worker threads to CPU 0 in every process. EAL's own threads
             * keep their affinities; restore only this caller. */
            cpu_set_t caller_cpus;
            if (sched_getaffinity(0, sizeof(caller_cpus), &caller_cpus) != 0) {
                int saved_errno = errno;
                npu_nvme_cleanup(ctx); return -saved_errno;
            }
            int env_rc = spdk_env_init(&env_opts);
            int affinity_rc = sched_setaffinity(0, sizeof(caller_cpus), &caller_cpus);
            if (env_rc >= 0) {
                spdk_inited = 1;
                initialized_role = role;
            }
            if (affinity_rc != 0) {
                int saved_errno = errno;
                fprintf(stderr, "[Fatal] Unable to restore caller CPU affinity.\n");
                npu_nvme_cleanup(ctx); return -saved_errno;
            }
            if (env_rc < 0) {
    ...
    if (role != NPU_NVME_ROLE_COPY_RANK) {
    ...
    if (role != NPU_NVME_ROLE_HOST_OWNER) {
    /* Initialise NPU environment */
    ret = aclrtSetDevice(ctx->acl.npu_id);
    ...
    for (int i = 0; i < ctx->dma.max_pipe_depth; i++) {
        ...
        ret = role==NPU_NVME_ROLE_HOST_OWNER ? ACL_SUCCESS : aclrtCreateEvent(&ctx->acl.events[i]);
        if (ret != ACL_SUCCESS) {
...
int npu_nvme_init(NPUNVMEContext **out_ctx, const char *pci_addr, int npu_id,
                  int pipe_depth, uint32_t chunk_size, bool enable_profiling,
                  const char *prof_dir) {
    return initialize_role(out_ctx,pci_addr,npu_id,pipe_depth,chunk_size,
                           enable_profiling,prof_dir,NPU_NVME_ROLE_COMBINED);
}
...
int npu_nvme_init_ex(NPUNVMEContext **out_ctx, const char *pci_addr,
    ...
    *out_ctx=NULL;
    if (!options || options->struct_size!=sizeof(*options) || options->version!=1 ||
        options->role>NPU_NVME_ROLE_COPY_RANK || options->reserved ||
        options->pipe_depth>INT_MAX) return -EINVAL;
    if (options->role==NPU_NVME_ROLE_HOST_OWNER) {
        if (options->npu_id!=-1) return -EINVAL;
    } else if (options->npu_id<0 || options->npu_id>7) return -EINVAL;
    if (options->role==NPU_NVME_ROLE_COPY_RANK && pci_addr && pci_addr[0]) return -EINVAL;
    return initialize_role(out_ctx,options->role==NPU_NVME_ROLE_COPY_RANK ? "copy-only" : pci_addr,
        options->npu_id,options->pipe_depth,options->chunk_size,false,prof_dir,options->role);
}
