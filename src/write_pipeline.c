...
void initiate_write_fsm(NPUNVMEContext *ctx, write_request_t *req) {
    ...
    fsm->next_spdk_submit_idx = req->resume_index;
...
void write_fsm_tick(NPUNVMEContext *ctx) {
    ...
    unsigned submit_budget = EFFECTIVE(ctx->submit_items_per_tick,4);
    int quantum=EFFECTIVE(ctx->quantum_items,DEFAULT_QUANTUM(ctx));
    int quantum_end=req->num_tasks;
    if (req != &fsm->faf_req && quantum_end-req->resume_index>quantum)
        quantum_end=req->resume_index+quantum;
    ...
    if (ctx->qpair && !test_fault("NPU_NVME_TEST_HOLD_NVME_COMPLETIONS"))
    ...
    for (int i = fsm->next_spdk_submit_idx; i < fsm->next_submit_idx; ++i) {
        ...
        if (ret != ACL_SUCCESS) {
            ...
            if (test_fault("NPU_NVME_TEST_FAIL_EVENT_SYNC")) sync_ret = ACL_ERROR_FAILURE;
    ...
    while (fsm->next_spdk_submit_idx < fsm->next_submit_idx) {
        ...
        if (task->state == CHUNK_HOST_COPYING) {
            size_t bytes=task->size-task->host_copy_offset;
            if (bytes>copy_budget) bytes=copy_budget;
            memcpy((char *)ctx->dma.pool[task->buf_idx].buf+task->host_copy_offset,
                   (char *)task->npu_ptr+task->host_copy_offset,bytes);
        ...
        if (task->state == CHUNK_NPU_DONE) {
            ...
            if (req->is_copy) {
                size_t bytes=task->size-task->host_copy_offset;
                if (bytes>copy_budget) bytes=copy_budget;
                memcpy((char *)task->host_ptr+task->host_copy_offset,
                       (char *)ctx->dma.pool[task->buf_idx].buf+task->host_copy_offset,bytes);
                copy_budget-=bytes;task->host_copy_offset+=bytes;
                if (task->host_copy_offset<task->size) break;
                task->state=CHUNK_DONE;fsm->completed_count++;
                release_dma_slot(ctx,task->buf_idx);dma_inflight_dec(ctx);
                if (ctx->enable_profiling) task->ts_slot_release=get_time_us();
                fsm->next_spdk_submit_idx++;continue;
            }
    ...
    while (fsm->next_submit_idx < quantum_end && submit_budget--) {
    ...
    if (fsm->completed_count==quantum_end && quantum_end<req->num_tasks) {
        rotate_request(ctx,0,req,quantum_end);
        fsm->req=NULL; fsm->state=WRITE_FSM_IDLE; return;
    }

    /* 5. Check if all chunks are complete. */
    if (fsm->completed_count >= req->num_tasks) {
        req->ts_batch_end = get_time_us();
        if (!req->is_copy) ctx->last_write_io_us = req->ts_batch_end - req->ts_batch_start;
...
int write_fsm_poller_fn(void *arg) {
    ...
    if (ctx->app_should_stop && fsm->state == WRITE_FSM_IDLE &&
        !ctx->ready_head[0] && !spdk_ring_count(ctx->write_ring)) return -1;

    /* Phase 1: check for new requests from Python (ring). */
    if (fsm->state == WRITE_FSM_IDLE) {
        write_request_t *req=take_ready_request(ctx,0);
        if (req) initiate_write_fsm(ctx,req);
    }
    ...
    if (fsm->state == WRITE_FSM_RUNNING) {
        ...
        if (fsm->state == WRITE_FSM_IDLE && atomic_load(&finished_req->done)) {
            ...
            if (!was_faf) { forget_accepted_write(ctx, finished_req); write_request_put(finished_req); }
        }
    }
