...
void forget_accepted_write(NPUNVMEContext *ctx, write_request_t *req) {
    if (req->is_copy) return;
    pthread_mutex_lock(&ctx->state_lock);
    if (req->result && !ctx->prior_write_error) ctx->prior_write_error=req->result;
    write_request_t **link=&ctx->accepted_writes;
    while (*link && *link!=req) link=&(*link)->accepted_next;
    if (*link) *link=req->accepted_next;
    req->accepted_next=NULL;
    pthread_mutex_unlock(&ctx->state_lock);
}

int enqueue_request(NPUNVMEContext *ctx, int direction, void *obj) {
    int rc = -ESHUTDOWN;
    write_request_t *request=obj;
    pthread_mutex_lock(&ctx->state_lock);
    if (!atomic_load(&ctx->app_should_stop) && !atomic_load(&ctx->admission_closed) &&
        ...
        if (ring) {
            if (atomic_load(&ctx->pending_requests)>=EFFECTIVE(ctx->max_pending_requests,64)) { rc=-EBUSY; goto finish; }
            if (direction==2 && request->is_flush) {
                request->dependency_error=ctx->prior_write_error;
                uint32_t count=0;
                for (write_request_t *p=ctx->accepted_writes;p;p=p->accepted_next) ++count;
                if (count) {
                    request->dependencies=calloc(count,sizeof(*request->dependencies));
                    if (!request->dependencies) { rc=-ENOMEM; goto finish; }
                    for (write_request_t *p=ctx->accepted_writes;p;p=p->accepted_next) {
                        atomic_fetch_add(&p->refs,1);
                        request->dependencies[request->dependency_count++]=p;
                    }
                }
            }
            if (direction==0 && !request->is_copy) { request->accepted_next=ctx->accepted_writes; ctx->accepted_writes=request; }
            ...
            if (spdk_ring_enqueue(ring, &obj, 1, NULL) == 1) rc = 0;
                ...
                if (direction == 0) {
                    ...
                    if (!request->is_copy) {
                        ctx->accepted_writes=request->accepted_next; request->accepted_next=NULL;
                    }
                }
                rc = -EBUSY;
            }
...
/* Rotate only after all commands in the quantum returned their DMA slots.
 * Alternating ring admission and owned-ready work prevents either source from
 * starving. The accepted request reference remains owned throughout rotation. */
write_request_t *take_ready_request(NPUNVMEContext *ctx, int direction) {
    void *obj=NULL;
    struct spdk_ring *ring=direction ? ctx->read_ring : ctx->write_ring;
    if ((!ctx->prefer_ready[direction] || !ctx->ready_head[direction]) &&
            spdk_ring_dequeue(ring,&obj,1)==1) {
        ctx->prefer_ready[direction]=true;
        if (!direction) atomic_fetch_sub(&ctx->queued_writes,1);
        return obj;
    }
    write_request_t *req=ctx->ready_head[direction];
    if (req) {
        ctx->ready_head[direction]=req->ready_next;
        if (!ctx->ready_head[direction]) ctx->ready_tail[direction]=NULL;
        req->ready_next=NULL; ctx->prefer_ready[direction]=false;
        return req;
    }
    if (spdk_ring_dequeue(ring,&obj,1)==1) {
        ctx->prefer_ready[direction]=true;
        if (!direction) atomic_fetch_sub(&ctx->queued_writes,1);
    }
    return obj;
}
void rotate_request(NPUNVMEContext *ctx, int direction, write_request_t *req, int prefix) {
    req->resume_index=prefix; req->ready_next=NULL;
    if (ctx->ready_tail[direction]) ctx->ready_tail[direction]->ready_next=req;
    else ctx->ready_head[direction]=req;
    ctx->ready_tail[direction]=req;
}
...
void *reactor_loop(void *arg) {
    ...
    /* Ring and admission limits are distinct; reserve a full pending window. */
    size_t ring_size=2;
    while (ring_size<=EFFECTIVE(ctx->max_pending_requests,64)) ring_size*=2;
    ctx->write_ring = spdk_ring_create(SPDK_RING_TYPE_MP_SC, ring_size,
                                        SPDK_ENV_SOCKET_ID_ANY);
    ctx->read_ring  = spdk_ring_create(SPDK_RING_TYPE_MP_SC, ring_size,
                                        SPDK_ENV_SOCKET_ID_ANY);
    ctx->meta_ring  = spdk_ring_create(SPDK_RING_TYPE_MP_SC, ring_size,
                                        SPDK_ENV_SOCKET_ID_ANY);
    ...
    while (!ctx->app_should_stop ||
           ctx->write_fsm.state != WRITE_FSM_IDLE ||
           ctx->read_fsm.state != READ_FSM_IDLE ||
           ctx->meta_req != NULL || ctx->ready_head[0] || ctx->ready_head[1]) {
