...
int npu_nvme_set_probe_flag_ptr(NPUNVMEContext *ctx, void *dev_ptr) {
    if (ctx && ctx->role!=NPU_NVME_ROLE_COMBINED) return -ENOTSUP;
    if (!ctx) return -1;
...
int step_poller_fn(void *arg) {
    ...
    if (cur_step > ctx->last_step_seen &&
        ...
        for (int i = 0; i < ctx->listener.num_registered_tasks; i++) {
            ctx->listener.registered_tasks[i].checksum_offset = 0;
            ctx->listener.registered_tasks[i].checksum_done = false;
        ...
        ctx->write_fsm.faf_req.async_dma = true;
