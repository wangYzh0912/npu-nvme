...
void write_profiling_csv(NPUNVMEContext *ctx, io_task_t *tasks,
    ...
    if (dir == PIPELINE_WRITE) {
        ...
        fprintf(f, "item,buf_idx,ts_read_submit_us,ts_nvme_done_us,ts_h2d_submit_us,"
                   "ts_h2d_done_us,ts_slot_release_us\n");
        for (int i = 0; i < num_items; ++i) {
            fprintf(f, "%d,%d,%lu,%lu,%lu,%lu,%lu\n", i, tasks[i].buf_idx,
                    tasks[i].ts_spdk_submit, tasks[i].ts_spdk_done,
                    tasks[i].ts_h2d_submit, tasks[i].ts_npu_done, tasks[i].ts_slot_release);
        }
    }
    fclose(f);
}
