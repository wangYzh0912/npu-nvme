...
void read_fsm_tick(NPUNVMEContext *ctx) {
    ...
    while (fsm->next_copy_idx < fsm->next_submit_idx) {
        ...
        if (task->state==CHUNK_HOST_COPYING && req->is_copy) {
            ...
            memcpy((char *)ctx->dma.pool[task->buf_idx].buf+task->host_copy_offset,
                   (char *)task->host_ptr+task->host_copy_offset,bytes);
    ...
    if (fsm->next_submit_idx < quantum_end) {
        ...
        if (task->size == 0 || aligned_sz > ctx->dma.chunk_size) {
            ...
            if (req->is_copy) {
                task->state=CHUNK_HOST_COPYING;fsm->next_submit_idx++;
                return;
            }
    ...
    if (fsm->completed_count >= req->num_tasks) {
        ...
        if (!req->is_copy) ctx->last_read_io_us = req->ts_batch_end - req->ts_batch_start;
