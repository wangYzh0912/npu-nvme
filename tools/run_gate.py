...
def main(argv=None):
    ...
    sources = {}
    for directory in ('python', 'src', 'include', 'tests', 'tools', 'config', 'experiments'):
        for path in sorted((ROOT/directory).rglob('*')):
            if path.is_file() and path.suffix in ('.py','.c','.h','.json'):
                sources[str(path.relative_to(ROOT))] = sha256_file(path)
    sources['train.py'] = sha256_file(ROOT/'train.py')
    sources['CMakeLists.txt'] = sha256_file(ROOT/'CMakeLists.txt')
    sources['src/exports.map'] = sha256_file(ROOT/'src/exports.map')
    write(out/'source_manifest.json', sources)
    results = []
    env = dict(os.environ, PYTHONPATH=str(ROOT)+':'+str(ROOT/'python'))
    for index, spec in enumerate(profile['cases']):
        run = out/f'case-{index:03d}'
        run.mkdir()
        junit = run/'junit.xml'
        command = [sys.executable,'-m','pytest','-q','--disable-warnings',spec['nodeid'],
                   '--junitxml',str(junit)]
    ...
    result = dict(schema_version=1,profile_id=profile['profile_id'],profile_sha256=sha256_file(out/'profile.json'),
                  ...
                  environment={'commit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),
                               'python':sys.version,'executable':sys.executable,'binary_sha256':None,
