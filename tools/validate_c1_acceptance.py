...
def validate_hardware(root, formal=True):
    ...
    for row in result['runs']:
        ...
        transport=config['checkpoint']['transport']
        expected_transport=transport if method=='ours' or transport=='legacy_sync' else 'framework'
        require(source['deterministic']=='ON' and source['transport']==expected_transport,'wrong execution mode')
        ...
        if method=='ours':
            require(read(root/'preflight.json')['library_sha256'] in [v for k,v in resources['loaded_libraries'].items() if Path(k).name == 'libnpu_nvme.so' or Path(k).name.startswith('libnpu_nvme.so.')],'loaded native binary differs')
