...
def collect_restore_timing(out):
    rows=[json.loads((out/f'rank_{rank}/restore-timing.json').read_text()) for rank in range(4)]
    if any(row['boundary']!='restore_begin_to_global_ready' or row['mandatory_integrity'] is not True
           or row['global_ready_ns']<=row['begin_ns'] for row in rows):
        raise ValueError('invalid rank restore timing')
    write(out/'timing.json',dict(boundary='restore_begin_to_global_ready',mandatory_integrity=True,
        seconds=(max(row['global_ready_ns'] for row in rows)-min(row['begin_ns'] for row in rows))/1e9,
        ranks=rows,clock='same-host monotonic_ns',includes=rows[0].get('includes')))
...
def main(argv=None):
    ...
    if a.method=='ours':
        ...
        if a.source_run and result['status']=='pass':collect_restore_timing(a.out)
    ...
    try:
        ...
        if a.source_run:collect_restore_timing(a.out)
