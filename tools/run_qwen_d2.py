...
def main(argv=None):
    ...
    owner_args=['tools/d2_session_owner.py','--operation',operation,'--out',str(a.out/'owner'),'--socket',socket_path,
        '--library',library,'--identity',str(identity_path),'--epoch',epoch,'--request-id',epoch,'--shm-id',str(a.shm_base),'--timeout',str(a.timeout)]
    if a.source_run:
    ...
    try:
        exits=[training.wait(timeout=max(.001,deadline-time.monotonic())),owner.wait(timeout=max(.001,deadline-time.monotonic()))]
