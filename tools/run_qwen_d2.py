...
def main(argv=None):
    ...
    try:
        while training.poll() is None:
            retained=[]
            for rank in range(4):
                path=a.out/f'rank_{rank}'/'d2-session-status.json'
                if path.exists() and json.loads(path.read_text()).get('status')=='retained':retained.append(rank)
            if retained:
                record.update(status='retained',retained_ranks=retained,
                    alive=[c.pid for c in (owner,training) if c.poll() is None]);save();return 3
            if time.monotonic()>=deadline:raise subprocess.TimeoutExpired(training.args,a.timeout)
            time.sleep(.2)
        training_exit=training.returncode
