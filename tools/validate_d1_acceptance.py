...
def validate(software, h01, h02, lifecycle):
    ...
    require(sw['profile_id']=='D1' and not sw['changed_sources'], 'wrong/moving software profile')
    ...
    for folder, name in ((h01,'sources.json'),(lifecycle,'sources.json')):
        ...
        required={k for k in sources if k.startswith(('python/','src/','include/','tests/')) and Path(k).suffix in ('.py','.c','.h')}
