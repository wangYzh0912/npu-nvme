...
def main():
    ...
    report['cpu_affinity_before']=sorted(os.sched_getaffinity(0))
    ...
    try:
        ...
        report['cpu_affinity_after']=sorted(os.sched_getaffinity(0))
        if report['cpu_affinity_after']!=report['cpu_affinity_before']:raise RuntimeError('native init changed caller CPU affinity')
        ...
        session=OwnerSession(region,connections,epoch=a.epoch,request_id=a.request_id,
            ...
            authoritative_schema=json.loads(a.strategy.read_text()) if a.strategy else None)
