...
from user_environment import (isolated_environment, read_profile,
                              verify_library_paths, verify_runtime_abi, with_python_identity)
...
def main():
    ...
    parser.add_argument("--library", type=Path, help="explicit stage build; included in environment identity")
    parser.add_argument("command", nargs=argparse.REMAINDER)
    args = parser.parse_args()
    try:
        profile = read_profile(args.manifest, args.profile, ROOT, library_override=args.library)
        profile = with_python_identity(profile)
        env = isolated_environment(profile, ROOT)
        libraries = verify_library_paths(profile, env)
        runtime = verify_runtime_abi(profile, env)
    ...
    if args.inspect:
        ...
        print(json.dumps({**profile, "paths": {k: env[k] for k in keys},
                          "resolved_libraries": libraries, "runtime_abi": runtime}, indent=2))
        return
    argv = args.command
    if argv[:1] == ["--"]:
        argv = argv[1:]
