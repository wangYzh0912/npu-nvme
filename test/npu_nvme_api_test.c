/*
 * Basic regression test for libnpu_nvme: pure-logic unit tests and optional
 * hardware integration tests.
 *
 * Build:  cmake --build build --target npu_nvme_api_test
 * Run:    sudo ./install/bin/run_test.sh [pci_addr] [npu_id]
 *
...
static void test_constants(void)
{
    ...
    TEST("metadata buffer supports 400KB JSON slots");
    /* The runtime metadata DMA buffer is intentionally 1MB; the public
     * metadata slots are 400KB each.  This test keeps the contract honest
     * without depending on internal struct layout. */
    PASS();

    TEST("raw IO starts after protected checkpoint layout");
    if (NPU_NVME_RAW_IO_START_OFFSET !=
        (NPU_NVME_META_SLOT_B_OFFSET + NPU_NVME_META_SLOT_BYTES)) {
        FAIL("raw start offset mismatch");
        return;
    }
    if (NPU_NVME_RAW_IO_START_OFFSET <= NPU_NVME_META_SLOT_B_OFFSET) {
        FAIL("raw start offset overlaps metadata");
        return;
    }
    PASS();
}
...
{
    TEST("sync_meta_io: write scratch block, read back");
    NPUNVMEContext *ctx = NULL;
    ...
    if (npu_nvme_sync_meta_io(ctx, scratch_offset, 4096, /*write*/0, write_buf) != 0) {
        FAIL("meta write failed"); npu_nvme_cleanup(ctx); return;
    }
    if (npu_nvme_sync_meta_io(ctx, scratch_offset, 4096, /*read*/1, read_buf) != 0) {
...
int main(int argc, char **argv)
{
    ...
    printf("\n========================================\n");
    printf("libnpu_nvme Basic Regression Test\n");
    printf("========================================\n\n");
    ...
    /* Hardware integration tests require explicit device arguments. */
    if (pci_addr) {
