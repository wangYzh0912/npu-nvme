...
typedef struct NPUNVMEContext {
    ...
    int app_should_stop;        /* accessed with acquire/release atomics */

    /* ---- Step-counter poller ---- */
    struct spdk_poller *step_poller;
    struct spdk_poller *write_fsm_poller;  /* async write FSM */
    struct spdk_poller *read_fsm_poller;   /* async read FSM */
    struct spdk_poller *meta_poller;       /* async metadata I/O */
    int last_step_seen;
    ...
    /* ---- Raw IO layout guardrails ---- */
    uint64_t raw_io_start_offset; /* first byte available to public raw IO */
    uint64_t raw_io_end_offset;   /* exclusive upper bound for public raw IO */
} NPUNVMEContext;
