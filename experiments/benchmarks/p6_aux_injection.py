...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--modes",nargs="+",default=("none","cpu","npu_serial","npu_offset","npu_parallel")); p.add_argument("--tasks",nargs="+",default=("diff","norm","topk","fp16","full")); p.add_argument("--mode",choices=("none","cpu","npu_serial","npu_offset","npu_parallel"),default=None,help="run one configuration in an isolated process"); p.add_argument("--task",choices=("diff","norm","topk","fp16","full"),default=None,help="run one configuration in an isolated process"); p.add_argument("--seeds",nargs="+",type=int,default=(41,42,43)); p.add_argument("--warmups",type=int,default=10); p.add_argument("--steps",type=int,default=30); p.add_argument("--seq-len",type=int,default=129); p.add_argument("--topk",type=int,default=128); p.add_argument("--npu",type=int,default=7); p.add_argument("--pci",default="0000:83:00.0"); p.add_argument("--output-root",default=None); p.add_argument("--model",choices=("gpt2","gpt2_xl"),default="gpt2_xl"); args=p.parse_args()
    ...
    for seed in args.seeds:
      ...
      for mode in modes:
       for task in tasks:
        bundle=EvidenceBundle("P6",{"model":args.model,"seed":seed,"mode":mode,"auxiliary":task,"warmups":args.warmups,"samples":args.steps},root=args.output_root,repo_root=ROOT,environment=environment_snapshot(pci=args.pci,npu=str(args.npu),repo_root=ROOT,npu_info=command(["npu-smi","info"])))
        values=[]; aux_values=[]
        try:
        ...
        result=bundle.finalize(metrics={"model":args.model,"seed":seed,"mode":mode,"latency_mean":stats(values),"foreground_wait":stats(aux_values),"auxiliary":task,"parallel_semantics":"separate host thread; profiler must confirm device overlap" if mode=="npu_parallel" else "serial"},status="pass" if len(values)==args.steps and not bundle.failures else "fail")
