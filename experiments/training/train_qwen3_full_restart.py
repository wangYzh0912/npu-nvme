    ...
    try:
        ...
        class Progress(ms.Callback):
            def on_train_begin(self, run_context):
                ...
                control=json.loads((source_rank/'control-checkpoint.json').read_text())
                ...
                readback=capture_control(ms,step=checkpoint_step,
                    data_sha256=report['data_sha256'],lr_horizon=lr_horizon,small=small)
                readback=json.loads(json.dumps(readback))
                if readback != control:
                    raise ValueError('restored control/RNG readback differs')
