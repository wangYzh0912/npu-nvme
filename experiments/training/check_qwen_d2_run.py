...
def audit(out,source=None):
    ...
    for rank in range(4):
        ...
        if report['status']!=expected or report.get('checkpoint_backend')!='d2':raise ValueError('D2 rank did not finish')
        ...
        if receipt['state_bytes']<=0 or receipt['capture']!='blocking':raise ValueError('missing explicit state capture')
    ...
    if not source:
        contract=dict(world_size=4,step=report['schedule']['checkpoint_step'],lr_horizon=report['schedule']['lr_horizon'],
