...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['param_data', 'param_info']
    output_names = ['delta_norms']
...
def _build_args(param_data_in__, param_info_in__, delta_norms_out_):
    __inputs__ = []
    for arg in [param_data_in__, param_info_in__]:
        if arg != None:
...
@tbe_register.register_operator("DeltaDetect", trans_bool_to_s8=False)
...
def delta_detect(param_data_in__, param_info_in__, delta_norms_out_, kernel_name="delta_detect", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(param_data_in__, param_info_in__, delta_norms_out_)
    ...
    origin_func_name = "delta_detect"
    ascendc_src_dir_ex = "delta_detect"
    ascendc_src_dir = "delta_detect"
    ascendc_src_file = "delta_detect.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator DeltaDetect. kernel name is " + kernel_name)
    op_type = "DeltaDetect"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[param_data_in__, param_info_in__], origin_outputs = [delta_norms_out_],\
...
def op_select_format(param_data_in__, param_info_in__, delta_norms_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(param_data_in__, param_info_in__, delta_norms_out_)
    result = check_op_cap("op_select_format", "DeltaDetect", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(param_data_in__, param_info_in__, delta_norms_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(param_data_in__, param_info_in__, delta_norms_out_)
    result = check_op_cap("get_op_specific_info", "DeltaDetect", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
