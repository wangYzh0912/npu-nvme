#!/usr/bin/env python3
"""
B3: Full FaF Baseline — ProbeTrainOneStepCell + DirectCheckpoint + Active SPDK.
Goal: measure per-step time WITH full FaF SPDK checkpoint writes.

This is the production configuration:
  - step_counter auto-increments in fused graph
  - C-layer listener polls step_counter every 10ms
  - On step % 10 == 0, launches 3.1GB SPDK write pipeline (asynchronous)
  - Python reads probe_flag to verify completion
...
Output: experiments/baselines/b3_faf_full_baseline.json
...
print("=" * 60)
print("  B3: Full FaF (SPDK active, ckpt_interval=10)")
print(f"  sink=TRUE s={SINK_SIZE}, {TOTAL_STEPS} steps")
print("=" * 60)
...
print("[B3] SPDK init (full FaF mode)...", flush=True)
...
print(f"[B3] flag=0x0 (self-alloc), step_counter={hex(dev_step)}", flush=True)
...
print(f"[B3] Tasks registered. flag={hex(dev_flag) if dev_flag else '0x0'}", flush=True)
...
class CB(ms.Callback):
    ...
    def on_train_epoch_end(self, rc):
        ...
        # Read probe flag to see how many checkpoints completed
        try:
            flag_val = ckpt.read_probe_flag_dev()
            flag_str = f"flag={flag_val}"
        except Exception:
            flag_str = "flag=N/A"
        print(f"  [B3] Epoch {epoch_num:2d}/{EPOCHS} | {et:.0f}ms epoch | "
              f"~{per_step:.0f}ms/step | {flag_str}", flush=True)
...
try:
    ...
    safety = "PASSED" if final_flag >= expected else f"FAILED({final_flag}<{expected})"
except Exception as e:
    final_flag = -1
    safety = f"ERR:{e}"

print(f"\n{'='*60}")
print(f"  B3 Results: Full FaF (SPDK active)")
print(f"{'='*60}")
...
result = {
    ...
    "final_flag": int(final_flag) if isinstance(final_flag, int) else str(final_flag),
    "safety": safety,
}
with open(REPO + "/experiments/baselines/b3_faf_full_baseline.json", "w") as f:
    json.dump(result, f, indent=2)
print(f"\n  Results saved to experiments/baselines/b3_faf_full_baseline.json")
