...
当前状态：执行中，未完成项不计为通过。已完成 33/35 项覆盖。
...
| auxiliary | B1 | 1 | 742.408 | 775.666 | 776.430 | 3633.38% | 325.489 | True |
| auxiliary | K10 | 1 | 153.353 | 158.219 | 168.295 | 671.17% | 32.692 | True |
| auxiliary | K5 | 1 | 114.399 | 117.025 | 127.416 | 475.29% | 16.378 | True |
...
![memory-auxiliary.png](figures/memory-auxiliary.png)

## 已测链路分段

| 组 | 评分/ms | 全局选择/ms | 打包流/ms | D2H 流/ms | payload 校验/ms | 上次提交等待/ms | 参考推进/ms |
|---|---:|---:|---:|---:|---:|---:|---:|
| auxiliary:B1 | 0.000 | 0.000 | 6.200 | 188.350 | 2700.577 | 32798.319 | 173.778 |
| auxiliary:K10 | 1878.149 | 748.072 | 11.703 | 14.708 | 266.698 | 3530.680 | 26.977 |
| auxiliary:K5 | 1892.067 | 734.858 | 11.110 | 10.151 | 133.441 | 1442.021 | 19.237 |
| main:B1 | 0.000 | 0.000 | 12.471 | 441.006 | 5502.367 | 60721.170 | 439.640 |
| main:K10 | 2976.039 | 1548.085 | 未采集 | 未采集 | 547.812 | 7326.757 | 66.594 |
| main:K20 | 2974.633 | 1608.098 | 16.377 | 78.870 | 1095.563 | 13891.212 | 103.182 |
| main:K5 | 2953.015 | 1512.176 | 17.098 | 20.486 | 276.592 | 3309.774 | 43.834 |
...
auxiliary：已测 Top-K 中，即使用较慢的 B0 作为分母，最小减速仍为 475.29%。这些配置未满足 3% 资源预算。
...
| auxiliary:B1 | 3.747 | 0.000 | 0.250 | 3.747 | 5.720 | 31.727 |
| auxiliary:K10 | 3.747 | 0.000 | 0.250 | 0.449 | 6.002 | 31.722 |
| auxiliary:K5 | 3.747 | 0.000 | 0.250 | 0.235 | 6.213 | 31.723 |
...
auxiliary 直接把现有权重参考扩展为完整训练状态参考，沿用现有 workspace 假设，投影峰值为 31.302～33.417 GiB/卡。这是当前实现的扩展预算，不是优化后的理论最低内存需求。
...
## auxiliary：Top 10% 的前三项直接成本

| 阶段 | 每步/卡中位耗时/ms | rank 0 的 20 步累计/s | 与下一训练 step 重叠 | 降低 Top 比例的影响 |
|---|---:|---:|---|---|
| 等待上次保存并推进参考 | 3530.680 | 69.852 | 等待发生在下一次检测前；后台保存此前可能与训练重叠 | 通常减少输出、传输和参考推进，但不消除扫描 |
| 评分与片段汇总 | 1878.149 | 36.873 | 当前同步回调内为零 | 三档均扫描全部候选，不能同比降低 |
| 全局 Top-K 选择 | 748.072 | 16.373 | 当前同步回调内为零 | 候选数不变，元数据和全局排序基本保留 |

rank 0 的检查点请求生命周期累计 220.328 秒，与训练 step 回调区间交集 10.277 秒。请求触发包含等待先前请求的时间，因此生命周期可以重叠，不能累加为关键路径或解释为硬件并行率。

表中累计值是该实现阻止后续训练进入的回调耗时，不是将某阶段删去后训练一定减少的反事实估计。后台传输、通信及 Host 调度相互影响；累计消融给出固定输入下的增量证据，实际路径 profiler 给出同一时钟的活动证据。

Owner 端各段中位耗时（ms）：{"request_ready": 4354.08, "prefix_write": 32.432, "payload_receive_and_write": 2519.998, "flush_commit_descriptor_encoding": 551.747}。接收与写入包含 socket、校验、数据复制和原始存储；提交段包含描述符编码，不直接归因于 NVMe。
...
| 配置 | 评分/ms | 选择及结果处理/ms | 打包/ms | D2H/ms | 参考推进/ms |
|---|---:|---:|---:|---:|---:|
| ablation-5 | 994.312 | 0.601 | 1.955 | 47.456 | — |
| ablation-6 | 995.492 | 0.503 | 1.860 | 47.640 | 56.017 |
...
## 实际 Top 10% 保存路径的同次时间线

| 卡 | 阶段 | 中位耗时/ms | Cube-only | Vector-only | 两者 | 两者均无 |
|---|---|---:|---:|---:|---:|---:|
| 0 | capture | 269.791 | 0.00% | 0.00% | 0.00% | 100.00% |
| 0 | payload_checksum | 559.101 | 0.00% | 0.00% | 0.00% | 100.00% |
| 0 | score | 3020.861 | 0.00% | 1.02% | 0.00% | 98.98% |
| 0 | selection | 1597.972 | 0.00% | 0.00% | 0.00% | 100.00% |
| 0 | source_stable | 0.096 | 0.00% | 0.00% | 0.00% | 100.00% |
| 0 | wait_previous_and_reference | 8087.504 | 0.00% | 0.00% | 0.00% | 100.00% |
| 1 | capture | 258.427 | 0.00% | 0.00% | 0.00% | 100.00% |
| 1 | payload_checksum | 550.054 | 0.00% | 0.00% | 0.00% | 100.00% |
| 1 | score | 3039.364 | 0.00% | 1.00% | 0.00% | 99.00% |
| 1 | selection | 1504.344 | 0.00% | 0.00% | 0.00% | 100.00% |
| 1 | source_stable | 0.093 | 0.00% | 0.00% | 0.00% | 100.00% |
| 1 | wait_previous_and_reference | 8108.412 | 0.00% | 0.00% | 0.00% | 100.00% |
| 2 | capture | 269.396 | 0.00% | 0.00% | 0.00% | 100.00% |
| 2 | payload_checksum | 546.098 | 0.00% | 0.00% | 0.00% | 100.00% |
| 2 | score | 2996.714 | 0.00% | 1.00% | 0.00% | 99.00% |
| 2 | selection | 1528.455 | 0.00% | 0.00% | 0.00% | 100.00% |
| 2 | source_stable | 0.092 | 0.00% | 0.00% | 0.00% | 100.00% |
| 2 | wait_previous_and_reference | 8088.399 | 0.00% | 0.00% | 0.00% | 100.00% |
| 3 | capture | 262.548 | 0.00% | 0.00% | 0.00% | 100.00% |
| 3 | payload_checksum | 549.006 | 0.00% | 0.00% | 0.00% | 100.00% |
| 3 | score | 2944.248 | 0.00% | 1.03% | 0.00% | 98.97% |
| 3 | selection | 1617.489 | 0.00% | 0.00% | 0.00% | 100.00% |
| 3 | source_stable | 0.096 | 0.00% | 0.00% | 0.00% | 100.00% |
| 3 | wait_previous_and_reference | 8084.303 | 0.00% | 0.00% | 0.00% | 100.00% |

使用每步 Host 单调时钟/墙钟括号锚点映射到同次 profiler 的设备任务时间线，校验每个评分区间确实覆盖 Vector 任务。此采集不进入性能组计时；任务包络也不等于核数量或指令利用率。
